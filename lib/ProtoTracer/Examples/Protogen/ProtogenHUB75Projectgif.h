...
#include "../../Assets/Textures/Animated/Bonk.h"
...
private:
    ...
    BonkSequence gif = BonkSequence(Vector2D(140, 140), Vector2D(0, 0), 18.0f);
    const uint8_t BONK = 69;

    float bonkOpacity = 0.0f;
    ...
    void LinkControlParameters() override {//Called from parent
        ...
        AddParameter(BONK, &bonkOpacity, 20, IEasyEaseAnimator::InterpolationMethod::Linear);
    ...
    void BadAppleFace(){
        HideFace();
        AddParameterFrame(BONK, 1.0f);

        SetMaterialOpacity(gif, bonkOpacity);
        SetBackgroundMaterialOpacity(gif, bonkOpacity);
    }
...
public:
    ...
    void SelectFaceFromMorse(uint8_t code) {
        ...
        switch(code) {
            ...
            case 25: BadAppleFace();            break; // [Y] Oscilloscope
